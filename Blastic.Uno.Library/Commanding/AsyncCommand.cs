...
namespace Blastic.Commanding
{
	...
	public class AsyncCommand<T> : ICommand
	{
		...
		public void AddKeyboardAccelerator(VirtualKey key, VirtualKeyModifiers modifiers, IViewAware context)
		{
			context.View.Subscribe(x =>
			{
				...
				keyboardAccelerator.Invoked += async (sender, args) =>
				{
					await Execute(null);
				};

				x.KeyboardAccelerators.Add(keyboardAccelerator);
			});
		}
	...
	public static class AsyncCommandExtensions
	{
		...
		public static AsyncCommand WithKeyboardAccelerator(
			this AsyncCommand command,
			VirtualKey key,
			VirtualKeyModifiers modifiers,
			IViewAware context)
		{
			command.AddKeyboardAccelerator(key, modifiers, context);
			return command;
		}

		public static AsyncCommand<T> WithKeyboardAccelerator<T>(
			this AsyncCommand<T> command,
			VirtualKey key,
			VirtualKeyModifiers modifiers,
			IViewAware context)
